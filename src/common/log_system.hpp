...
public:
	static void initialized(){
		spdlog::info("日志初始化");
		//初始化日志信息
		s_logger = spdlog::daily_logger_mt(MS_LOGGER_NAME.string(), MS_FILENAME.string(), MS_HOUR, MS_MINUTE);
		//设置全局日志等级
...
private:
	...
	inline const static fs::path MS_LOGGER_NAME = Config::config_yaml["log"]["logger-name"].as<std::string>();
	inline const static fs::path MS_FILENAME = Config::config_yaml["log"]["filename"].as<std::string>();
